...
2. Search for your dream domain name (This could actually be the hardest part)
3. Once found, add it to your bucket and checkout.
...
Once you clicked on `[DNS]` it will open the DNS default setting. And `[Use the Google Domains name servers]` will be selected 
by default. 

Select `[Use custom name servers]` instead so you can customize the setting.
...
Click on `[Add custom domain]` then you will be prompted to enter the domain you want to use. 
This is where you enter the domain name you just purchased from Google. Then click verify.

Netlify will check if the domain is available, and since you purchased it from Google, it wont be available. 
...
This is the informations you will need to finalise your setting on Google side. As far as Netlify is concerned, you are done!
Yeah! told you, Netfliy is dead simple ...
...
Now back to Google domains for the final setup. All you have to do is to :
